...
const CategoryLinks = () => {
  // --- NEW: Updated with more attractive, high-quality images ---
  const categories = [
    {
      name: 'Cakes',
  ...
  return (
    // --- NEW: Semi-transparent "glassmorphism" background ---
    <div className="bg-white/60 backdrop-blur-md shadow-md">
      <div className="container mx-auto px-4 py-6">
        {/* --- NEW: Redesigned flex container for better spacing --- */}
        <div className="flex justify-center sm:justify-around items-center gap-4 sm:gap-6 flex-wrap">
          {categories.map((category) => (
            <Link to={category.path} key={category.name} className="group">
              {/* --- NEW: Attractive card design for each category --- */}
              <div className="flex flex-col items-center text-center p-4 w-28 h-36 bg-pink-50/70 rounded-2xl shadow-lg transition-all duration-300 ease-in-out hover:shadow-2xl hover:bg-pink-100/80 hover:-translate-y-2">
                <div className="w-20 h-20 mb-2">
                  <img src={category.image} alt={category.name} className="w-full h-full object-contain transition-transform duration-300 group-hover:scale-110" />
                </div>
