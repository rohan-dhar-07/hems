...
const BottomSection = () => {
  ...
  // --- FIX: Updated paths to match your App.jsx routes ---
  const categoryItems = [
  ...
  // This useEffect handles closing the menu when clicking outside
  useEffect(() => {
  ...
  // This useEffect handles the open/close animation
  useGSAP(() => {
    if (isCategoriesOpen) {
      // Animate the menu opening with a "water spread" effect
      gsap.fromTo(menuRef.current,
        { clipPath: 'circle(0% at 50% 100%)' }, // Start as a dot at the bottom-center
        { clipPath: 'circle(150% at 50% 100%)', duration: 0.5, ease: 'power3.out' } // Expand to a large circle
      );
    }
  }, [isCategoriesOpen]);
